...
const size_t global[2] = { max_size, max_size }; 
...
void setup_kernel_memory(int a[][MAX], int b[][MAX], int c[][MAX]);
...
int main(int argc, char *argv[]) {  
  ...
  if(process_id == MASTER) {
    ...
    row_processed_by_worker = MAX / num_node_workers;
    remainder_row = MAX % num_node_workers;
    row_number = 0;
    for(int dest = 1; dest <= num_node_workers; dest++) {
      ...
      // Send matrix A
      MPI_Send(&a[row_number][0], rows * MAX, MPI_INT, dest, MASTER_TAG, MPI_COMM_WORLD);
      // Send matrix B
      MPI_Send(&b, MAX * MAX, MPI_INT, dest, MASTER_TAG, MPI_COMM_WORLD);
    ...
    for(int i = 1; i <= num_node_workers; i++) {
      ...
      MPI_Recv(&c[row_number][0], rows * MAX, MPI_INT, i, WORKER_TAG, MPI_COMM_WORLD, &status);
  ...
  if(process_id != MASTER) {
    ...
    MPI_Recv(&a, rows * MAX, MPI_INT, MASTER, MASTER_TAG, MPI_COMM_WORLD, &status);
    MPI_Recv(&b, MAX * MAX, MPI_INT, MASTER, MASTER_TAG, MPI_COMM_WORLD, &status);
    ...
    MPI_Send(&c, rows * MAX, MPI_INT, MASTER, WORKER_TAG, MPI_COMM_WORLD);
  }
  MPI_Finalize();
